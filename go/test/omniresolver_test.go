...
import (
	...
	omni "github.com/voxgig-sdk/nasa-open-apis-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NasaOpenApisError); is {
				out := map[string]any{
					"name":    "NasaOpenApisError",
...
func omniCtx(arg any, client *sdk.NasaOpenApisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NasaOpenApisSDK); is {
			client = live
