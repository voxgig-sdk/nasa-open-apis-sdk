...
func TestMarsPhotoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NASAOPENAPIS_TEST_MARS_PHOTO_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func mars_photoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NASAOPENAPIS_TEST_MARS_PHOTO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NASAOPENAPIS_TEST_MARS_PHOTO_ENTID"])
