# NasaOpenApis Golang SDK



The Golang SDK for the NasaOpenApis API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.MarsPhoto(nil)` — each with the same small set of operations (`List`, `Load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```go
func NewNasaOpenApisSDK(options map[string]any) *NasaOpenApisSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *NasaOpenApisSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### NasaOpenApisSDK methods
...
| `MarsPhoto` | `(data map[string]any) NasaOpenApisEntity` | Create a MarsPhoto entity instance. |
| `Planetary` | `(data map[string]any) NasaOpenApisEntity` | Create a Planetary entity instance. |

### Entity interface (NasaOpenApisEntity)

All entities implement the `NasaOpenApisEntity` interface.
...
API path: `/mars-photos/api/v1/rovers/{rover}/photos`
...
Create an instance: `planetary := client.Planetary(nil)`
...
```go
planetary, err := client.Planetary(nil).Load(map[string]any{"api_key": "api_key"}, nil)
if err != nil {
    panic(err)
}
...
marsphoto.List(nil, nil)

// marsphoto.Data() now returns the marsphoto data from the last list
