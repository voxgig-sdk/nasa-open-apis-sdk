...
module NasaOpenApisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NasaOpenApisResponse.new({})
    ...
    if fetch_err
      ...
      response = NasaOpenApisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NasaOpenApisResponse.new(fetched)
    else
