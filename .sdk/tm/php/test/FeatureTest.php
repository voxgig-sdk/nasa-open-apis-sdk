...
class FtCtrl extends NasaOpenApisControl
{
...
class FtClient
{
    ...
    private NasaOpenApisUtility $utility;

    public function __construct(NasaOpenApisUtility $utility, array $options)
    {
    ...
    public function get_utility(): NasaOpenApisUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NasaOpenApisContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NasaOpenApisContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NasaOpenApisContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NasaOpenApisConfig::make_feature($name);
    ...
    private static function buildUrl(NasaOpenApisSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NasaOpenApisError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NasaOpenApisResponse($fetched);
        }
    ...
    private function fail(NasaOpenApisContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NasaOpenApisContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
