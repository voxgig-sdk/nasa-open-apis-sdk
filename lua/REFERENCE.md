# NasaOpenApis Lua SDK Reference

Complete API reference for the NasaOpenApis Lua SDK.
...
```lua
local results, err = client:MarsPhoto(nil):list(nil, nil)
```
...
```lua
local result, err = client:Planetary(nil):load({ id = "planetary_id" }, nil)
```
