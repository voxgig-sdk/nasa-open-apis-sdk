...
local NasaOpenApisSDK = {}
NasaOpenApisSDK.__index = NasaOpenApisSDK
...
NasaOpenApisSDK._make_feature = _make_feature


function NasaOpenApisSDK.new(options)
  local self = setmetatable({}, NasaOpenApisSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NasaOpenApisSDK:options_map()
...
function NasaOpenApisSDK:get_utility()
  return Utility.copy(self._utility)
end


function NasaOpenApisSDK:get_root_ctx()
  return self._rootctx
end


function NasaOpenApisSDK:prepare(fetchargs)
...
function NasaOpenApisSDK:direct(fetchargs)
...
-- by passing it.
function NasaOpenApisSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NasaOpenApisSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NasaOpenApisSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:MarsPhoto():list() / client:MarsPhoto():load({ id = ... })
...
function NasaOpenApisSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NasaOpenApisSDK.new(sdkopts)
  sdk.mode = "test"
