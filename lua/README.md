# NasaOpenApis Lua SDK



The Lua SDK for the NasaOpenApis API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:MarsPhoto()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local planetary, err = client:Planetary():load({ api_key = "api_key" })
```
...
```
lua/
├── nasa-open-apis_sdk.lua    -- Main SDK module
...
The main module (`nasa-open-apis_sdk`) exports the SDK constructor
