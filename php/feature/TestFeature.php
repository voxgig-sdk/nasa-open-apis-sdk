...
// NasaOpenApis SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NasaOpenApisTestFeature extends NasaOpenApisBaseFeature
{
    ...
    public function init(NasaOpenApisContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NasaOpenApisContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NasaOpenApisParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NasaOpenApisContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NasaOpenApisContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NasaOpenApisParam::call($ctx, $k);
