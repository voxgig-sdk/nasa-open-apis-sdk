...
// NasaOpenApis SDK retry feature
...
class NasaOpenApisRetryFeature extends NasaOpenApisBaseFeature
{
    ...
    public function init(NasaOpenApisContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NasaOpenApisContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NasaOpenApisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NasaOpenApisContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
