...
class MarsPhotoEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = NasaOpenApisSDK::test($seed, null);
        $seen = iterator_to_array($base->MarsPhoto(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = NasaOpenApisConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = NasaOpenApisSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->MarsPhoto(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NASA_OPEN_APIS_TEST_MARS_PHOTO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function mars_photo_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NASA_OPEN_APIS_TEST_MARS_PHOTO_ENTID");
