...
// NasaOpenApis SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NasaOpenApisOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(NasaOpenApisOmni::structfix($now, Struct::undef()))
                        !== json_encode(NasaOpenApisOmni::structfix($val, Struct::undef()))) {
