...
class MarsPhotoDirectTest extends TestCase
{
    public function test_direct_list_mars_photo(): void
    {
        $setup = mars_photo_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-mars_photo", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["rover01"] as $_liveKey) {
