# NasaOpenApis PHP SDK



The PHP SDK for the NasaOpenApis API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->MarsPhoto()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($marsphotos as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["camera"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $marsphoto));
```
...
```php
$client = NasaOpenApisSDK::test($testopts, $sdkopts);
```
...
| `MarsPhoto` | `($data): MarsPhotoEntity` | Create a MarsPhoto entity instance. |
| `Planetary` | `($data): PlanetaryEntity` | Create a Planetary entity instance. |
...
```php
// load() returns the ENTITY — call data_get() for the Planetary record (throws on error).
...
```
php/
├── nasaopenapis_sdk.php          -- Main SDK class
...
The main class (`nasaopenapis_sdk.php`) exports the SDK class
...
// $marsphoto->data_get() now returns the marsphoto data from the last list
