...
describe('PlanetaryDirect', async () => {
  ...
  test('direct-load-planetary', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-planetary', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NASA_OPEN_APIS_TEST_PLANETARY_ENTID']
