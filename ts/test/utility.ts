...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NASAOPENAPIS_TEST_LIVE ||
    'TRUE' === process.env.NASAOPENAPIS_TEST_OVERRIDE
  ) {
  ...
  m.NASAOPENAPIS_TEST_EXPLAIN = process.env.NASAOPENAPIS_TEST_EXPLAIN || m.NASAOPENAPIS_TEST_EXPLAIN
