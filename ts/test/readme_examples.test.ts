...
//     (\`new NasaOpenApisSDK(...)\` / \`NasaOpenApisSDK.test(...)\` become
//     \`NasaOpenApisSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NasaOpenApisSDK'
...
const TEST_SEED = {"entity":{"mars_photo":{"test01":{"id":"test01"}},"planetary":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NasaOpenApisSDK(...)\` and any
// \`NasaOpenApisSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NasaOpenApisSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NasaOpenApisSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
