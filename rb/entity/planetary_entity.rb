...
class PlanetaryEntity
  ...
  def make
    opts = @_entopts.dup
    PlanetaryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NasaOpenApisHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Planetary, Hash] the current Planetary data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NasaOpenApisHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Planetary fields)
  def match_get
  ...
  #
  # @param reqmatch [PlanetaryLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Planetary.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Planetary, Hash] the loaded Planetary; raises NasaOpenApisError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NasaOpenApisHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
