...
class MarsPhotoDirectTest < Minitest::Test
  def test_direct_list_mars_photo
    setup = mars_photo_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-mars_photo", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["rover01"].each do |_live_key|
...
def mars_photo_direct_setup(mockres)
