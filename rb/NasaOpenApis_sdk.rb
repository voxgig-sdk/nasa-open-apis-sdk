...
require_relative 'NasaOpenApis_types'
...
class NasaOpenApisSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NasaOpenApisHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NasaOpenApisHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NasaOpenApisFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NasaOpenApisUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NasaOpenApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NasaOpenApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NasaOpenApisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NasaOpenApisHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NasaOpenApisError.new(
        "graphql_error", "NasaOpenApisSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NasaOpenApisSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
