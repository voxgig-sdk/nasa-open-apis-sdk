...
[MCP](https://modelcontextprotocol.io) server exposing the NasaOpenApis SDK as
two agent tools — `nasa-open-apis_list` and `nasa-open-apis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nasa-open-apis-mcp)
make build

# 2. Provide credentials via the environment
export NASA_OPEN_APIS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nasa-open-apis \
  -- /absolute/path/to/nasa-open-apis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nasa-open-apis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nasa-open-apis_list: first page of records
{ "entity": "mars_photo" }
{ "entity": "mars_photo", "query": { } }

// nasa-open-apis_load: one record by id
{ "entity": "planetary", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nasa-open-apis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nasa-open-apis \
     -- "$PWD"/dist/*/nasa-open-apis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nasa-open-apis_list` and `nasa-open-apis_load` tools now appear
   in new sessions. Ask the agent to *"list mars_photo using nasa-open-apis"*
   and it calls `nasa-open-apis_list` with `{"entity":"mars_photo"}`.
...
```sh
export NASA_OPEN_APIS_APIKEY=sk_live_xxx            # API key
export NASA_OPEN_APIS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nasa-open-apis_list` tool
...
### Call the `nasa-open-apis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nasa-open-apis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nasa-open-apis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NASA_OPEN_APIS_APIKEY` | API key sent with every request. |
| `NASA_OPEN_APIS_BASE` | Optional override of the API base URL. |
...
```sh
./nasa-open-apis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nasa-open-apis_load","arguments":{"entity":"planetary","query":{"id":1}}}}'
```
