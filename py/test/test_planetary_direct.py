...
class TestPlanetaryDirect:

    def test_should_direct_load_planetary(self):
        setup = _planetary_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-planetary", "live" if setup["live"] else "unit")
        if _skip:
...
def _planetary_direct_setup(mockres):
