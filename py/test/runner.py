...
class NasaOpenApisTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NasaOpenApisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NasaOpenApisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NasaOpenApisTestRunner.getenv("NASA_OPEN_APIS_TEST_LIVE")
        override = NasaOpenApisTestRunner.getenv("NASA_OPEN_APIS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NasaOpenApisTestRunner.getenv(key)
        ...
        explain = NasaOpenApisTestRunner.getenv("NASA_OPEN_APIS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NasaOpenApisTestRunner._test_control is not None:
            return NasaOpenApisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NasaOpenApisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NasaOpenApisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NasaOpenApisTestRunner.load_test_control()
...
def load_env_local():
    NasaOpenApisTestRunner.load_env_local()


def env_override(m):
    return NasaOpenApisTestRunner.env_override(m)


def entity_data(v):
    return NasaOpenApisTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NasaOpenApisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NasaOpenApisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NasaOpenApisTestRunner.load_test_control()


def live_client_options():
    return NasaOpenApisTestRunner.live_client_options()


def live_delay_ms():
    return NasaOpenApisTestRunner.live_delay_ms()
