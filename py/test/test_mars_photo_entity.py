...
class TestMarsPhotoEntity:
    ...
    def test_should_stream(self):
        ...
        base = NasaOpenApisSDK.test(seed, None)
        seen = list(base.MarsPhoto(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NasaOpenApisSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.MarsPhoto(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NASA_OPEN_APIS_TEST_MARS_PHOTO_ENTID JSON to run live")
        client = setup["client"]
...
def _mars_photo_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NASA_OPEN_APIS_TEST_MARS_PHOTO_ENTID"))
