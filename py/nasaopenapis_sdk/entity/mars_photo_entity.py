...
from nasaopenapis_sdk.nasaopenapis_types import (
    MarsPhoto,
    MarsPhotoListMatch,
)
...
class MarsPhotoEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MarsPhotoEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[MarsPhoto]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.MarsPhoto().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
