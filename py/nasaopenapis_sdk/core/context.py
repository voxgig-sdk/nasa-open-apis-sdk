...
from nasaopenapis_sdk.core.helpers import get_ctx_prop, to_map
...
class NasaOpenApisContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, NasaOpenApisSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, NasaOpenApisResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, NasaOpenApisResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return NasaOpenApisError(code, msg, self)
